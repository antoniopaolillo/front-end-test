...
import { filterByName } from 'services/filterServices';
...
export const TableProvider: React.FC<Props> = (props: Props) => {
  ...
  const [filteredData, setFilteredData] = useState<null | PlanetInterface[]>(null);
  ...
  useEffect(() => {
    if(initialData) {
      setFilteredData(filterByName(initialData || [], filters.filterByName.name));
    }
  }, [filters, initialData]);
  return (
